...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const path = '/services/aluminum-cutting';
  ...
  return generateSEO({
    ...
    locale,
    alternateLanguages,
    ogImage: '/og-aluminum.jpg',
    keywords,
  });
}

// İkonlar sırasıyla c1_t..c4_d ile eşleşir
const iconMap = [Target, Zap, Shield, Clock];
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-gray-500 font-light text-lg mb-8 leading-relaxed">{t('mat_desc')}</p>
            </div>
            <div className="grid sm:grid-cols-2 gap-3">
              {materials.map((mat, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-gray-100 hover:border-indigo-200 hover:shadow-md transition-all"
                >
