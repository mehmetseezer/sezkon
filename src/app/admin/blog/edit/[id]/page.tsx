...
import { analyzeSEO as runSEOAnalysis } from '@/lib/seo-analyzer';
import {
  ArrowLeft,
  Save,
  Send,
...
Summary (2–3 sentences)`;
  ...
  useEffect(() => {
    ...
    fetch(`/api/blog/${id}`)
      ...
      .catch(() => setError('Blog verisi yüklenemedi'))
  ...
  // SEO Analysis Logic
  const analyzeSEO = () => {
    return runSEOAnalysis(form);
  };

  if (fetching) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <Loader2 className="animate-spin text-[#6191c4]" size={32} />
      </div>
    );
  }
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-base font-extrabold text-slate-800 hidden sm:block">Blog Yazısını Düzenle</h1>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">Güncelle ve Yayınla</span>
              <span className="inline sm:hidden">Güncelle</span>
            </button>
          </div>
        </div>
      </nav>
        ...
        {validationErrors.length > 0 && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-xl px-5 py-4">
            <p className="text-sm font-extrabold text-red-700 mb-2 flex items-center gap-2">
              <XCircle size={16} /> Güncelleme ve yayınlamak için lütfen aşağıdaki eksiklikleri giderin:
            </p>
